...
REPLy 0.1.0. The complete implementations live in `middleware/custom.jl` in this
repository, with a QA suite in `qa/middleware.py`.
...
1. **Intercept** — handle an op and return without calling `next`
2. **Transform** — modify the request, call `next`, return the result
3. **Augment** — call `next`, inject extra response messages, return
...
> **This rule has no exceptions.** Omitting the type annotation produces a
> runtime method ambiguity error on the first request.
...
**Consequence:** If middleware A forwards a request with a *different op* to
be handled by middleware B, then **A must be placed before B** in the stack.
...
This is the most common source of silent failures when building middlewares that
...
Note: `ctx.session` is `nothing` for ephemeral evals (no `session` field in
the request) and for ops that don't route through `SessionMiddleware`.
...
appear **before** the terminal response in the final output, regardless of
when `emit!` is called.
...
> **Note:** The comprehension `string(k) => v for (k,v) in pairs(msg)` is
> necessary because `JSON3.Object` keys are `Symbol`-like, not `String`.
> A future REPLy version should expose a `mutable_copy(msg)` helper.
...
```julia
# IncludeFixMiddleware: rewrites bare include("f") → Base.include(@__MODULE__, "f")
# Must be placed BEFORE EvalMiddleware in the stack.
```

See the full implementation above (§ Modifying a Request Before Forwarding).
...
**Wire it:** Near the top of the stack (before `SessionMiddleware`) so it
wraps everything, including session routing overhead.
...
- Check for errors by scanning `"error" in r.get("status", [])` on the returned
  response vector
...
These symbols are exported from REPLy and safe to use from custom middleware:
...
**Internal (not exported, do not depend on):**
...
| `_run_eval_core` | Full I/O capture for arbitrary code | Forward as `eval` op, or use `eval_responses` if accessible |
...
`AuditMiddleware` exists but is not in the default stack. Wire it in to get
a rotating JSON log of every operation:
...
| `emit!` messages appear but in the wrong order | `emit!` always prepends to `ctx.emitted` which comes before terminal | Expected behaviour — use `emit!` only for side-channel data, not the primary response |
...
**`next(msg)` is a stack cursor, not a restart.**
Think of `next` as "continue from my position". If you change `op` in the
message, only middlewares *below you in the stack* will see the new op. If the
handler for that op is *above you*, it will never receive it.

**Outer middlewares see everything; inner middlewares see less.**
A middleware at position 1 times every op including `ping` and `metrics`. A
middleware at position 8 (after `SessionMiddleware`) only sees ops that passed
session validation. Place observability concerns near the top, domain logic
near its natural position.

**`emit!` is for side-channel data, not the primary response.**
The primary response comes from the return value of `handle_message`. Use
`emit!` for annotations (timing, metrics, warnings) that should accompany any
response without changing its semantics.

**One middleware, one responsibility.**
Julia's multiple dispatch makes it tempting to handle five ops in one
`handle_message`. Resist: `validate_stack` gets confused, stack traces become
opaque, and testing individual ops becomes harder. One struct per logical concern.

---

## Further Exploration

- `middleware/custom.jl` — complete implementations of all six patterns
- `middleware_demo/server.jl` — wiring a custom stack end-to-end
- `qa/middleware.py` — Python test suite for each middleware
- `~/.julia/packages/REPLy/.../src/middleware/` — all built-in middleware
  source (audit, eval, load_file, session_ops, stdin, interrupt) — the
  definitive reference for how REPLy itself uses the pattern
